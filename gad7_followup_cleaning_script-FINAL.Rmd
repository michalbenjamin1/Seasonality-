---
title: "GAD7 Follow Up Cleaning Script"
...
# Read in the data: GAD 7 follow up data 
...
## gad_coping_followupa.rds
```{r gad7 coping follow up a read in data}
gad_cop_fua_dat <- readRDS("/seasonal_internalising_2024/gad_coping_followupa.rds")
...
```{r gad7 coping follow up a select}
gad_cop_fua_dat_id <- gad_cop_fua_dat %>% #new dataset with ID
  drop_na(externalDataReference) %>% # Drop participants with no ID
  #remove_duplicates("externalDataReference") %>% # Remove duplicate IDs
  add_column(sample = "gad7_fua",
  ...
  select(
         ...
         startDate,
         endDate,
         gad7.feeling_nervous_anxious_or_on_edge_cop = gad7.feeling_nervous_anxious_or_on_edge,
         gad7.control_worrying_stop_cop = gad7.control_worrying_stop,
         gad7.worrying_too_much_about_different_things_cop = gad7.worrying_too_much_about_different_things,
         gad7.trouble_relaxing_cop = gad7.trouble_relaxing,
         gad7.sit_restless_hard_cop = gad7.hard_sit_restless,
         gad7.becoming_easily_annoyed_or_irritable_cop = gad7.becoming_easily_annoyed_or_irritable,
         gad7.awful_feeling_afraid_happen_cop = gad7.awful_happen_feeling_afraid,
         gad7.difficult_daily_life_issues_cop = gad7.difficult_daily_life_issues
         )
  # %>%
...
colslogic<-!(colnames(gad_cop_fua_dat_id) %in% exclude_cols_numeric)
colnames(gad_cop_fua_dat_id)[colslogic]<-paste0(colnames(gad_cop_fua_dat_id)[colslogic],"_numeric")
...
```{r gad7 coping follow up a number excluded}
...
gad_cop_fua_excluded <- dim(gad_cop_fua_dat_id)[1] - dim(gad_cop_fua_dat)[1]
gad_cop_fua_excluded
```

Inspect numeric variables
```{r gad7 coping follow up a inspect numeric variables}
gad_cop_fua_dat_id %>%
...
```{r gad7 coping follow up a inspect missingness}
gad_copa_fua_miss_map <- gad_cop_fua_dat_id %>% 
  missmap()
gad_copa_fua_miss_map
```

## gad_coping_followupb.rds
```{r gad7 coping follow up b read in data}

gad_cop_fub_dat <- readRDS("/users/k21043154/project/seasonal_internalising_2024/data/phenotypic/raw_data_old/gad_coping_followupb.rds")
...
Select & rename relevant columns (will be a function at some point)
+++JZ: We do not need individually named variables right now as these can be be filtered by the sample, and will force us to harmonise naming across follow up groups.
```{r gad7 coping follow up b select}
gad_cop_fub_dat_id <- gad_cop_fub_dat %>% #new dataset with ID
  drop_na(externalDataReference) %>% # Drop participants with no ID
  #remove_duplicates("externalDataReference") %>% # Remove duplicate IDs
  add_column(sample = "gad7_fub",
             .after = "externalDataReference") %>% # Create new sample column
  select(
         ID = externalDataReference,# ID
         sample,
         ...
         endDate,
         gad7.feeling_nervous_anxious_or_on_edge_cop = gad7.feeling_nervous_anxious_or_on_edge,
         gad7.control_worrying_stop_cop = gad7.stop_control_worrying,
         gad7.worrying_too_much_about_different_things_cop = gad7.worrying_too_much_about_different_things,
         gad7.trouble_relaxing_cop = gad7.trouble_relaxing,
         gad7.sit_restless_hard_cop = gad7.hard_sit_restless,
         gad7.becoming_easily_annoyed_or_irritable_cop = gad7.becoming_easily_annoyed_or_irritable,
         gad7.awful_feeling_afraid_happen_cop = gad7.awful_feeling_afraid_happen,
         gad7.difficult_daily_life_issues_cop = gad7.difficult_daily_life_issues
         )
  # %>%
  #add_labelled_numeric(exclude = exclude_cols_numeric, leaders = exclude_cols_numeric)
...
#+++JZ: fix to numeric columns to follow the old standard of gad7 cleaning
colslogic<-!(colnames(gad_cop_fub_dat_id) %in% exclude_cols_numeric)
colnames(gad_cop_fub_dat_id)[colslogic]<-paste0(colnames(gad_cop_fub_dat_id)[colslogic],"_numeric")
...
Look at number of people excluded
```{r gad7 coping follow up b number excluded}
# Inspect dimensions of new data set
gad_cop_fub_dat_id %>%
  dim()
...
# Inspect number of rows dropped
gad_cop_fub_excluded <- dim(gad_cop_fub_dat_id)[1] - dim(gad_cop_fub_dat)[1]
gad_cop_fub_excluded
```

Inspect numeric variables
```{r gad7 coping follow up b inspect numeric variables}
gad_cop_fub_dat_id %>%
  select(all_of(ends_with("numeric"))) %>%
  tbl_summary(missing_text = "Missing")
```
...
Check missingness by missmap
```{r gad7 coping follow up b inspect missingness}
gad_cop_fub_miss_map <- gad_cop_fub_dat_id %>% 
  missmap()
gad_cop_fub_miss_map
```

## gad_coping_followupa_ongoing.rds
```{r gad7 coping follow up a ongoing read in data}
gad_cop_fua_o_dat <- readRDS("/users/k21043154/project/seasonal_internalising_2024/data/phenotypic/raw_data_old/gad_coping_followupa_ongoing.rds")
...
Select & rename relevant columns (will be a function at some point)
+++JZ: We do not need individually named variables right now as these can be be filtered by the sample, and will force us to harmonise naming across follow up groups.
```{r gad7 coping follow up a ongoing select}
gad_cop_fua_o_dat_id <- gad_cop_fua_o_dat %>% #new dataset with ID
  drop_na(externalDataReference) %>% # Drop participants with no ID
  #remove_duplicates("externalDataReference") %>% # Remove duplicate IDs
  add_column(sample = "gad7_fua_o",
             .after = "externalDataReference") %>% # Create new sample column
  select(
         ID = externalDataReference,# ID
         sample,
         ...
         gad7.worrying_too_much_about_different_things_cop = gad7.worrying_too_much_about_different_things,
         gad7.trouble_relaxing_cop = gad7.trouble_relaxing,
         gad7.sit_restless_hard_cop = gad7.sit_hard_restless,
         gad7.becoming_easily_annoyed_or_irritable_cop = gad7.becoming_easily_annoyed_or_irritable,
         gad7.awful_feeling_afraid_happen_cop = gad7.awful_feeling_afraid_happen,
         gad7.difficult_daily_life_issues_cop = gad7.difficult_daily_life_issues
         )
...
#+++JZ: fix to numeric columns to follow the old standard of gad7 cleaning
colslogic<-!(colnames(gad_cop_fua_o_dat_id) %in% exclude_cols_numeric)
colnames(gad_cop_fua_o_dat_id)[colslogic]<-paste0(colnames(gad_cop_fua_o_dat_id)[colslogic],"_numeric")

  
# Inspect colnames
gad_cop_fua_o_dat_id %>%
  colnames()
```

Look at number of people excluded
```{r gad7 coping follow up a ongoing number excluded}
# Inspect dimensions of new data set
gad_cop_fua_o_dat_id %>%
  dim()
...
# Inspect number of rows dropped
gad_cop_fua_o_excluded <- dim(gad_cop_fua_o_dat_id)[1] - dim(gad_cop_fua_o_dat)[1]
gad_cop_fua_o_excluded
```

Inspect numeric variables
```{r gad7 coping follow up a ongoing inspect numeric variables}
gad_cop_fua_o_dat_id %>%
  select(all_of(ends_with("numeric"))) %>%
  tbl_summary(missing_text = "Missing")
```
...
Check missingness by missmap
```{r gad7 coping follow up a ongoing inspect missingness}
gad_copa_fua_o_miss_map <- gad_cop_fua_o_dat_id %>% 
  missmap()
gad_copa_fua_o_miss_map
```
...
```{r Merge data sets}
nrow(gad_cop_fua_dat_id)
nrow(gad_cop_fub_dat_id)
nrow(gad_cop_fua_o_dat_id)
dat <- bind_rows(gad_cop_fua_dat_id,gad_cop_fub_dat_id, gad_cop_fua_o_dat_id)
...
+++JZ: This is short now because all followup 'samples' use the same variable names
```{r Create cont vars vector}
variables_cont <- c(
  "gad7.feeling_nervous_anxious_or_on_edge_cop_numeric",
  "gad7.control_worrying_stop_cop_numeric",
  "gad7.worrying_too_much_about_different_things_cop_numeric",
  "gad7.trouble_relaxing_cop_numeric",
  "gad7.sit_restless_hard_cop_numeric",
  "gad7.becoming_easily_annoyed_or_irritable_cop_numeric",
  "gad7.awful_feeling_afraid_happen_cop_numeric",
...
#me attempting to get the dates back to posix format
#dat2[,variables_date]<-lapply(as.POSIXct(dat2[,variables_date], format = "%d/%m/%y %H:%M:%S")
...
dat %>% 
  saveRDS(
    file = file.path(("/users/k21043154/project/seasonal_internalising_2024/data/phenotypic/raw_data_old/gad_coping_followupall_clean.rds")
    )
```
...
dat %>% 
  filter(sample == "gad7_fua") %>%  # select only follow up A data
  saveRDS(
    file = file.path(ilovedata,"data","latest_freeze","coping_glad_edgi_nbr","clinical", "gad7_followup_a_clean.rds")
    )
```
...
```{r Write cleaned follow up a variables to a .rds file}
dat %>% 
  filter(sample == "gad7_fub") %>%  # select only follow up B data
  saveRDS(
    file = file.path(ilovedata,"data","latest_freeze","coping_glad_edgi_nbr","clinical","gad7_followup_b_clean.rds")
    )
```
...
```{r Write cleaned follow up a variables to a .rds file}
dat %>% 
  filter(sample == "gad7_fua_o") %>%  # select only follow up B data
  saveRDS(
     file = file.path(ilovedata,"data","latest_freeze","coping_glad_edgi_nbr","clinical","gad7_followup_a_ongoing_clean.rds")
    )
```
